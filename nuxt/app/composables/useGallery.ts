...
export const useGallery = () => {
  ...
  // 全局缓存状态
  const galleriesCache = useState<GalleryItem[] | null>('galleries-cache', () => null)
  const galleriesCacheTime = useState<number>('galleries-cache-time', () => 0)
  ...
  const getGalleries = async (forceRefresh = false): Promise<GalleryItem[] | null> => {
    ...
    try {
      const result = await $fetch<GalleryItem[]>(`${baseURL}/gallery`)
  ...
  const getAllGalleries = async (): Promise<GalleryItem[]> => {
    return await $fetch<GalleryItem[]>(`${baseURL}/gallery/admin`, {
      headers: {
